...
char** split(char* string,char* delimiter,int* ctArgs)
{
	...
	char* ptr = strtok(string,delimiter);//fist call: strtok must be initialized with a string
	int position = 0;	
	int chunks =0;
	 while(ptr)
	 {
	    ...
	    //if size of buffer exceed, reallocate:
            if(position >= BUFFER_SIZE)
	      {
             ...
             ptr = strtok(NULL,delimiter);//striktok is already initailized, hence we pass NULL to it
	  }
          *ctArgs = position;
	return buffer;
}
...
{  
        ...
        else if(pid==0)
	{
	    ...
	    //We have to terminate the passed argumnets with a NULL pointer which must be cast to char*
